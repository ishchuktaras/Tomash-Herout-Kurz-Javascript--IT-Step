...
function cenik(){
//    precteni cookie
    susenky_retezec = document.cookie;
    susenky_pole = susenky_retezec.split("; ");
    let kosik1 = '';
    let y = '';
    for(let i = 0; susenky_pole.length > i; i++){
        y = susenky_pole[i].split("=");
        if(y[0] == 'kosik1'){
           kosik1 =  y[1];
        }
    }
    ...
    $('#cenik-form input[type="number"]').blur(function(){
       ...
       celkem = celkem.toString();
       celkem = celkem.split("");
       celkem = celkem.reverse();
        let celkem_text = '';
        for(let i = 0; i < celkem.length; i++){
            if(i == 3 || i == 6){
                celkem_text = celkem_text + ' ';
            }
                celkem_text = celkem_text + celkem[i]; 
        }
        celkem_text = celkem_text.split("").reverse().join("");
       $('#celkem').text(celkem_text);
       document.cookie = 'kosik1='+celkem_text+'; expires=Thu, 18 Dec 2023 12:00:00 UTC';
